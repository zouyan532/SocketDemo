...
public class MsgView {
    ...
    public Map<String, Object> parseRespond(String data){
        ...
        for(String tmp : buff){
            //获取key值
            int index = TextUtils.indexOf(tmp, "@=");
            if(index==-1){
                String temp = ((String)tmp).replaceAll("@S", "/").replaceAll("@A", "@");
                this.parseRespond(temp);
            }else{
                String key = TextUtils.substring(tmp, 0,index );
                //获取对应的value值
                Object value = TextUtils.substring(tmp,index + 2, tmp.length());
